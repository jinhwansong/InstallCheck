...
export function openModelImport(onImport:(model:ImportedModel)=>boolean) {
  ...
  form.onsubmit=e=>{
    ...
    try {const model=prepareMesh(raw,Number(unit.value),up.value as 'Y'|'Z');if(onImport({asset:{id:crypto.randomUUID(),name:filename,positions:model.positions},size:model.size,role:role.value as 'equipment'|'factory'}))dialog.close();}
